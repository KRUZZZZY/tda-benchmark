#!/usr/bin/env python3
"""Expansion #5 residual (c) — 9-dataset PANEL subset, repeated 5-fold CV,
25 repetitions.

The 9-dataset panel (scripts/sweep_multidataset.py) was single-split.
This driver brings a representative 2-dataset subset of the panel to the
repeated-CV protocol of scripts/sweep_repeated_cv_r25.py (expansion plan
#5): one time-series dataset (ecg200, the paper's headline) and one image
dataset (10-class MNIST-1000, the panel's image arm).

  * ecg200  (data/tda/ucr/ecg200_{X,y}.npy, time_series, Takens d=3 tau=1)
    filtrations {vietoris_rips, weak_alpha} (the TS pairing; weak_alpha
    completes on ECG200)
  * mnist10 (data/tda/images/mnist10_1000_{X,y}.npy, image, 1000 samples)
    filtrations {cubical, vietoris_rips} (the image pairing)
...
Additive-only: creates NEW DB data/tda/r25_panel.db only.

Usage:
  cd /home/kruzzzzy/Documents/AI_KOS_PROJECT
  .venv-tda/bin/python projects/tda-benchmark/scripts/sweep_r25_panel.py

Expected runtime: 1-2 h serial.
DB: ../../data/tda/r25_panel.db
"""
...
DB_PATH = str(PROJECT_ROOT / "data" / "tda" / "r25_panel.db")
...
def stage_stats(db_path: str) -> None:
    ...
    if len(reps) < 2:
        ...
        return
    # row: 0=dataset, 1=rep, 2=fil, 3=vec, 4=clf, 5=acc
...
def main() -> None:
    ...
    jobs = []
    for fil, vec, clf in product(TS_FILS, VECTORIZATIONS, CLASSIFIERS):
        for rep in range(1, REPETITIONS + 1):
            jobs.append((ECG200, fil, vec, clf, rep))
    for fil, vec, clf in product(IMG_FILS, VECTORIZATIONS, CLASSIFIERS):
        for rep in range(1, REPETITIONS + 1):
            jobs.append((MNIST10, fil, vec, clf, rep))
    jobs = [j for j in jobs
            if (j[0].name, j[1].name, j[2].name, j[3].name, j[4]) not in done]
