...
3--21\% faster than Vietoris-Rips at identical accuracy on
...
For $n=100$ (points per cloud after subsampling) and
$\sigma=0.15$, this gives $d_B \lesssim 0.46$; at $\sigma=0.30$,
$d_B \lesssim 0.91$. We compare
...
\begin{itemize}
  ...
  \item \textbf{Persistence and stability} (\S2.3--\S2.4): The
    probabilistic bound $d_B \lesssim \sigma\sqrt{2\log n}$ predicts
    that $\sigma = 0.15$ noise perturbs diagrams by $\lesssim 0.46$,
    while torus $\Betti_1$ features have persistences of the order of
    $0.4$--$1.2$ (measured). \S4.2 confirms that classification
    survives this perturbation.
...
\begin{table}[H]
  ...
  \footnotesize{All $p$-values are reported descriptively; no multiplicity
    correction is applied across the three stage comparisons.}
\end{table}
...
(the weakest). Cubical filtration leads at 73.7\%, with Sparse Rips (73.3\%)
and Vietoris-Rips (73.3\%) within 0.02pp of each other and Weak Alpha
(73.0\%) 0.3pp lower.
...
is also not distinguishable. On MNIST binary, Persistence Statistics reaches 95.9\% marginal
accuracy, within 2.1pp of the best configuration overall (Cubical +
Betti Curve + Logistic Regression at 98.0\%, Table~\ref{tab:pareto}). All comparisons
...
we use the probabilistic bound of \S2.4: with high probability the
bottleneck distance between clean and noisy diagrams is at most
$O(\sigma\sqrt{\log n})$, i.e.\ $d_B \lesssim 0.46$ at
$\sigma=0.15$ and $\lesssim 0.91$ at $\sigma=0.30$ for $n=100$.
...
respectively). The probabilistic stability bound at
$\sigma = 0.15$ is $d_B \lesssim 0.46$, but the torus's $\Betti_1$ features have
measured top-$H_1$ persistences span roughly $0.4$--$1.2$ on the
clean torus clouds --- features near the lower
...
$O(\sigma\sqrt{\log n})$ for the sphere and torus geometries tested.

At $\sigma = 0.30$ (bound $\lesssim 0.91$), mean accuracy across the 112
...
Alpha is 3--21\% faster than VR at identical accuracy on
sphere/torus with Landscape+SVM (measured 3.5--3.9s vs.\ 3.9--4.4s
across the four noise levels; the gap narrows at $\sigma=0.30$). Cubical filtration is the
...
rows, so the pixel-grid topology is absent from the outset. Conti et al.\ (2022) showed that this
...
and Weak Alpha faithfully capture the embedded point cloud's topology.
...
argument applies to the full complex, and the approximation is standard
practice for point-cloud filtrations.)
...
$\sim$88\%, DTW-based 1-NN $\sim$88\% on the standard UCR train/test
...
in scale as well as topology (torus point norms $\in [1,3]$ vs.\
sphere $\equiv 1$; $\approx$10\% of torus points have norm $<1.1$),
so a radial-threshold classifier would already separate most of the
data; the noise experiments therefore measure robustness of the
\emph{combined} geometric signal, not topology alone. (2)~Homology
capped at $H_1$ ($H_2$ requires
$O(n^4)$ simplices for VR). (3)~Two-class problems only.
(4)~One real dataset per modality. (5)~Single implementation library
...
binary 0/1 subset \cite{lecun1998}. The SQLite result database is
regenerated by the bundled \texttt{run\_all.sh} script. Random seeds
are fixed (base 42) and per-dataset subsampling is seeded
deterministically, so the sweep is reproducible end-to-end.
...
median 1.4s); Sparse Rips dominates the total (140 runs, mean 41s,
$\approx$1.6h), while the other three filtrations complete in
$\approx$24 minutes. Per-configuration wall times are recorded in the
results database (cf. the R-package PH runtime benchmark of
Somasundaram et al.\ \cite{somasundaram2021}); memory was not
measured per stage (see Limitations).
...
\begin{table}[H]
  ...
  \small
    ...
    \midrule
    Low-dim.\ point clouds ($\le 3$D, clean) & Alpha complex & Pers.\ Statistics or Landscapes & SVM (RBF) or Logistic & Alpha 3--21\% faster than VR at identical accuracy; Nerve Theorem guarantees homotopy fidelity \\
