...
widely cited answer, from Conti et al.\ (2022), is that filtration
...
(extensible from 3, 4, and 3 via YAML-only changes).
...
cubical filtration was incompatible with point-cloud data). The
topological signal survives Gaussian noise up to $\sigma = 0.30$
at 99.85\% mean accuracy: classification accuracy survives noise
levels where the stability bound's worst-case guarantee would permit
degradation, and the $\Betti_1 = 0$ vs.\ $\Betti_1 = 2$ distinction
pipelines. On MNIST, cubical filtration outperforms Vietoris-Rips
by 1.8 percentage points (98.0\% vs.\ 96.25\%), confirming that
filtration choice matters for image data. The Alpha complex is
20--30\% faster than Vietoris-Rips at identical accuracy on
...
classification --- each with 5--15 available methods. The literature's
most widely cited answer to ``which stage matters most?'' comes from
Conti et al.\ (2022), who showed that filtration choice on MNIST
...
simultaneously in a controlled factorial sweep: a 180-configuration
core sweep (5 datasets $\times$ 3 filtrations $\times$ 4 vectorizers
$\times$ 3 classifiers), extended to 616 configurations with a fourth
filtration (cubical), three additional vectorizers (Silhouette,
Amplitude, Persistence Entropy), and a fourth classifier (SVM-linear).
The framework is extensible to 7 filtrations, 11 vectorizers, and 4
classifiers via YAML-only configuration changes.
...
\begin{enumerate}
  \item \textbf{Finding.} On ECG200 time series with Takens embedding,
    vectorization dominates classification accuracy (1.9pp marginal
    range, 95\% CI [1.2, 2.5], Wilcoxon $p = 0.038$) while filtration
    is statistically neutral (0.1pp, 95\% CI [0.0, 0.4], $p = 0.61$).
  ...
  \item \textbf{Framework.} The finding was produced by a benchmarking
    ...
    3 filtrations, 4 vectorizers, and 3 classifiers executed in a
    180-configuration factorial sweep, with YAML-driven configuration
...
\begin{theorem}[Nerve Theorem --- Leray 1945]
  \label{thm:nerve}
...
\begin{theorem}[Stability Theorem --- Cohen-Steiner et al.\ 2007]
  \label{thm:stability}
...
\begin{itemize}
  \item \textbf{Simplicial complex size} (\S2.1--\S2.2): VR's
    ...
    20--30\% wall-clock speed gap reported in \S4.3. The Nerve
    Theorem guarantees that Alpha's speed advantage does not cost
    geometric fidelity.
  \item \textbf{Filtration choice} (\S2.2): VR, Alpha, and Sparse
    Rips constitute the three filtrations varied in the factorial
    sweep of \S4.1.
...
\begin{itemize}
  \item \textbf{Embed} (optional, time series only). Maps a 1D
    ...
    \begin{equation}
      ...
      \label{eq:takens}
    \end{equation}
    By Takens' Delay Embedding Theorem (1981), if the signal $x$
...
$n_{\text{target}} = 100$ are reduced via \emph{farthest point
sampling} (FPS): initialise $S = \{x_{i_1}\}$ with an arbitrary
seed point; iteratively add $x_{i_{k+1}} = \arg\max_{x \in X
\setminus S} \min_{s \in S} \|x - s\|$ until $|S| = n_{\text{target}}$.
FPS preserves the metric space geometry and Hausdorff distance
significantly better than uniform random sampling, directly
benefitting persistent homology output stability.
...
One benchmark sweep, three analytical cuts. The sweep comprises
180 configurations: 5 datasets $\times$ 3 filtrations
(Vietoris-Rips, Alpha, Sparse Rips) $\times$ 4 vectorizers
(Persistence Statistics as baseline; Persistence Image, Persistence
Landscape, Betti Curve in the factorial design) $\times$ 3
classifiers (SVM-RBF, Random Forest, Logistic Regression).
Persistence Statistics is included as a fourth vectorizer baseline
in the narrative but was evaluated in a reduced sweep alongside the
full factorial; its marginal accuracy was computed from the same
cross-validation folds. All
filtrations computed $H_0$ and $H_1$ homology. All configurations
used stratified 5-fold cross-validation with fixed random seed (42).
Point clouds were subsampled to 100 points via farthest point
...
100 points per cloud after FPS; $\Betti_1 = 0$ vs.\ $2$), and
...
kernel-based methods on ECG200: its marginal accuracy (75.0\%) is
within 1.0pp of Persistence Landscapes (76.0\%), a difference whose
95\% CI of [0.3, 2.1] and Wilcoxon $p = 0.038$ confirm Landscapes
do outperform but by a narrow margin. Statistics versus Images
(74.1\%) yields $p = 0.23$ (not distinguishable). On MNIST binary,
Persistence Statistics (97.0\%) is within 0.5pp of the best method
(Persistence Images at 97.5\% with Random Forest). All $p$-values
...
On synthetic sphere/torus data with $\sigma = 0.00$, all 27
configurations achieve 100\% accuracy. No stage dominates --- the
signal is strong enough that every pipeline separates it perfectly.
...
At $\sigma = 0.00$, $0.05$, and $0.15$, \textbf{every} configuration
achieves 100\% accuracy. The stability bound at $\sigma = 0.15$ is
...
At $\sigma = 0.30$ (bound $= 1.20$), mean accuracy across all 616
configurations remained at 99.85\%. The worst single configuration
(Sparse Rips + Amplitude + SVM-linear) still achieved 98.5\% —
...
Alpha is 20--30\% faster than VR at identical accuracy (2.7s vs.\
4.2s on sphere/torus with Landscape+SVM). Cubical filtration is the
...
28--33s with no accuracy gain at the 100-point scale; its design
...
Landscape + SVM-RBF is the Pareto-optimal default: 2.0--2.7s,
76\% on ECG200, 100\% on synthetic data, on the two datasets tested; the Pareto frontier may shift
...
\begin{table}[H]
  ...
  \begin{tabular}{lccc}
    ...
    Time series (embedded) & Vectorizer & 1.9pp & 1 \\
...
Our pipeline's best ECG200 accuracy (76.0\%) sits below published
classical baselines for this dataset: Euclidean 1-NN achieves
$\sim$88\%, DTW-based 1-NN $\sim$81\% on the standard UCR train/test
split. This is expected for a method using topology alone --- TDA
...
vectorizers (PersLay, Hofer input layers). (6)~$n=200$ produces wide
...
\begin{table}[H]
  ...
  \begin{tabular}{p{3.2cm}p{2.8cm}p{2.8cm}p{2.8cm}p{4cm}}
    \toprule
    Data Modality & Filtration & Vectorizer & Classifier & Rationale \\
    \midrule
    Low-dim.\ point clouds ($\le 3$D, clean) & Alpha complex & Pers.\ Statistics or Landscapes & SVM (RBF) or Logistic & Alpha 20--30\% faster than VR at identical accuracy; Nerve Theorem guarantees homotopy fidelity \\
    Delay-embedded time series ($d \ge 3$) & VR or Alpha & Pers.\ Landscapes & RF or SVM (RBF) & Vectorization dominates variance (1.9pp, $p=0.038$); Landscapes most stable across embeddings \\
    High-noise clouds ($\sigma \ge 0.15$) & Alpha or VR & Pers.\ Landscapes & SVM (RBF) & Landscapes most robust to geometric distortion at $\sigma=0.30$ (11.2pp vectorizer range) \\
...
Pipeline-stage importance in TDA classification is modality-dependent.
On ECG200 time series, vectorization produced a statistically significant
accuracy range (p < 0.05), while filtration did not (p > 0.5). This
...
99.85\% mean accuracy — the stability bound is conservative,
...
output:
  db_path: data/tda/full_results.db
  log_level: INFO
\end{lstlisting}
...
sweep reported in Chapter~4 exercises 3, 4, and 3 of these
