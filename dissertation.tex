...
vectorizers, and 4 classifiers (56 runs failed and were excluded) --- we find that the
...
vectorization moves accuracy by 6.39pp (95\% CI [6.13, 6.65]) versus
0.69pp for filtration (95\% CI [0.57, 0.81]); on binary MNIST the
...
\begin{enumerate}
  ...
  \item \textbf{Verified finding: vectorization dominates.} On ECG200
    ...
    classification accuracy: a 6.39pp marginal range across 7
  ...
  \item \textbf{Matched-scale synthetic evidence.} On synthetic
    ...
    not an artefact of the norm/scale confound
...
across six dataset instances and four analytical cuts.
...
benchmark. Under the 25-repetition repeated-CV protocol, raw-signal baselines
...
repetitions (83.2\% mean, $r{=}5$; \S4.1). As a robustness check, re-running the
...
Vectorization dominates on the full menu: its range (6.39pp across 7
...
vectorizers and matching the number of levels. By $\omega^2$ the
...
$-0.017$; MNIST $0.214$ versus $0.143$ and $0.032$), so its dominance
is not an artefact of level counts. The marginal-range \emph{size},
...
below the filtration range (3.60pp). The isolated contribution ---
...
(2.1pp). The honest reading is that vectorization is the largest
...
larger stage even on image data. This is not a single-split
...
under the design penalty: ECG200 vectorizer $\omega^2 = 0.165$
(bootstrap 95\% CI [0.063, 0.423]) versus classifier 0.077
([0.003, 0.246]) and filtration $-0.017$ ([$-0.017$, 0.093]);
MNIST vectorizer 0.214 ([0.103, 0.540]) versus filtration 0.143
...
for MNIST, $N{=}1400$). On ECG200 the two-way interactions are not
...
patient recording, BIDMC chf07) with the same 5-fold protocol on a
...
share is 28.0\%) and a pipeline subset (2 filtrations,
3 vectorizers, 2 classifiers; the subset spans the strongest and
...
class-balanced metrics too. Vectorization dominance on time series
is not an artefact of the specific ECG200 instance; a full
...
therefore partly grid-structure-driven: the time-window layout of the
...
$\sigma=0.30$) demonstrates the topological signal is not an
artefact of the norm marginal, which is the scoped claim. The
topological signal is therefore not an artefact of the norm
confound: it survives the norm/scale confound control, and it is
robust to noise at the
...
(22--31~s per configuration (12-config dedicated subprocess
measurements) vs.\ 0.7--9.0~s for the others); the bare
interpreter with numpy/sklearn/gtda/gudhi imported already uses
...
a classifier can use? On ECG200 the vectorizer range is 6.39pp
versus 0.69pp for filtration; on ECG5000 it is 24.89pp versus
3.60pp. These full-range figures are prior to the equal-footing
...
noise results measure robustness of
...
ranking therefore uses Vietoris-Rips only for the time-series arm;
...
configuration). The improvement is modest (0.35--1.72pp against the 25-repetition raw baselines)
...
TDA therefore contributes complementary signal rather than
...
benchmark (48 records, 47 patients; AAMI-5-style protocol with the Q (unknown) class excluded (4 classes: N, S, V, F; chance 25\%);
patient-disjoint 5-fold CV; beat windows of 128 samples at 360~Hz,
Takens-embedded to $126 \times 3$; 2000 beats class-balanced
...
the relevant result is the vectorizer ordering, which transfers. Notably, weak alpha fails on
...
robust cross-dataset property rather than an ECG5000 artefact.

\textbf{Window-length sensitivity.} To check that the vectorizer
ordering is not an artefact of the 128-sample window, the same
...
r25 numbers are the fixed-grid estimate). On the two-dataset panel
